import bg from '/images/map.webp';
...
const SignUp = () => {
    ...
    const { createUser, logOut, signInWithGoogle, loading, user, setLoading } = useAuth();
    ...
    return (
        ...
        >
            ...
            <div className=' relative z-10 min-h-screen md:w-3/5 lg:w-2/5 md:bg-[#161616] flex flex-col justify-center items-center w-[500px] h-[600px]'>
                ...
                <form onSubmit={handleSignUp} className="text-left flex justify-center flex-col items-center text-white ">
                    ...
                    <div className='w-[350px] md:w-[400px] '>
                        ...
                        <div className='flex flex-col space-y-5'>
                            <input required className='glass-morphism py-4 pl-4 pr-16 text-left outline-none rounded-sm font-raleway text-sm' placeholder='Email' type="email" name="email" />
                            <div className='relative'>
                                <input required className='glass-morphism py-4 pl-4  w-full pr-16 text-left outline-none rounded-sm font-raleway text-sm' placeholder='Password' type={passText ? "password" : "text"} name="password" />
                                <span onClick={() => setPassText(!passText)} className='absolute right-5 top-4'> {passText ? <FaLock /> : <FaLockOpen />} </span>
                            </div>
                            <div className='relative'>
                                <input required className='glass-morphism py-4 pl-4 w-full pr-16 text-left outline-none rounded-sm font-raleway text-sm' placeholder='Confirm Password' type={confirmPassText ? "password" : "text"} name="confirmPassword" />
                                <span onClick={() => setConfirmPassText(!confirmPassText)} className='absolute right-5 top-4'>{confirmPassText ? <FaLock /> : <FaLockOpen />}</span>
                            </div>
                            <span className=''> <SignButton name='Sign Up'></SignButton> </span>
                        </div>
                    </div>
